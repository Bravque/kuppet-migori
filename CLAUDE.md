...
18. `backend/config/migration-school-category-tertiary.sql` — adds `tertiary_school` to the `school_category` ENUM on **both** `members` and `bbf_claims` (third registration category). ⚠ **Needs running on live** (created 24 July 2026); folded into `init*.sql`. Without it, picking "Tertiary School" at registration → the DB rejects the ENUM value.
