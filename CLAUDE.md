...
26. `backend/config/migration-scholarship-timeline.sql` — creates `scholarship_application_timeline` (mirrors `bbf_claim_timeline`) so scholarship applications have the same status-change audit trail as BBF claims. ⚠ **Needs running on live** (created 26 July 2026; folded into `init*.sql`). **Deploy-safe without it:** all timeline reads/writes are best-effort (try/catch), so scholarship apply + the admin detail page keep working if the table is missing — the timeline just stays empty until the migration runs. Paired with the new `PUT /api/admin/scholarship-apps/:id/review` (Start Review, applied→under_review, `authorizeAdmin`) + `scholarship_under_review` notification, so the scholarship flow now matches BBF (submitted→under_review→approved/rejected→paid, each logged + notified).
25. `backend/config/migration-schools.sql` — creates the `schools` table (`name` UNIQUE, `sub_county`, `is_active`) **and seeds it from the distinct `members.school_name` values already on record**. ⚠ **Needs running on live** (created 26 July 2026; the CREATE is folded into `init*.sql` but the seed-from-members step only lives in the migration). Powers the school-name **autocomplete** on registration/profile/admin filters + the admin **Schools** page (`schools.html`). Public source at `GET /api/schools`; admin CRUD at `/api/admin/schools` (`schoolsController` + `routes/schools.js` public / `routes/adminSchools.js` admin). Members store `school_name` as free text (no FK), so editing/deleting a school only changes the suggestion list — existing members are untouched. **The curated list is the source of truth (26 July 2026):** member registration + profile-save now **reject a school that isn't on the active list** (via `backend/utils/schools.js` `resolveSchool`), storing the canonical spelling; off-list values get `OFF_LIST_MESSAGE` ("select from the list / contact the office"). `register.html` mirrors this client-side (`KNOWN_SCHOOLS` set, checked in `validateStep`). ⚠ **Deploy-safe empty-list guard:** if `schools` is empty or missing (migration #25 unrun), enforcement is skipped so it can never block all registrations — so **run migration #25 on live** for enforcement to actually take effect.
