...
**DB migrations — #1–27 all applied on live (last: #27 on 4 August 2026)**
...
**Document rules** — `BBF_DOC_SLOTS` in `memberBbfController` is the **single source of truth** for both which `doc_type`s a claim may carry and which are required to submit (28 July 2026); `submitClaim` derives its required list *and* its labels from it, and it mirrors `BBF_DOC_SLOTS_DEATH`/`_RETIREMENT` in `member-portal.js` — keep the two in step. **death** → TSC Slip, Burial Permit, Letter from Principal (+ optional Birth Notification); **retirement** → TSC Slip + Letter of Compulsory Retirement. `letter_of_compulsory_retirement` was added to the `bbf_claim_documents.doc_type` ENUM in migration #19; admin `bbf-detail.html` `DOC_LABELS` maps it too. `uploadDocuments` enforces:
