...
23. `backend/config/migration-scholarship-amount.sql` — adds `scholarship_applications.amount_awarded DECIMAL(12,2) NULL` (award amount set by admin on approval, mirrors `bbf_claims.amount_approved`). ⚠ **Needs running on live** (created 24 July 2026); folded into `init*.sql`. Without it, approving a scholarship 500s (the UPDATE sets `amount_awarded`).
24. `backend/config/migration-scholarship-paid.sql` — adds `paid` to `scholarship_applications.status` ENUM + `payment_reference VARCHAR(100)` + `payment_date DATE` (admin "mark as paid" flow, mirrors BBF). ⚠ **Needs running on live** (created 24 July 2026); folded into `init*.sql`. Without it, marking a scholarship paid 500s.
