...
npm run init-db               # Create MySQL schema (31 tables) and seed data
...
| MySQL schema (31 tables) + seed data | ✓ Complete |
...
**DB migrations — #1–17 all applied on live (20 July 2026)**
Scripts #1–17 have been run on the live Hostinger DB and are folded into `init.sql` / `init-hostinger.sql` for fresh installs — their individual `backend/config/migration-*.sql` files are kept for re-provisioning reference (see `docs/SESSION-NOTES.md` for what each changed). Reference of the most recent:
...
21. `backend/config/migration-transactional-templates.sql` — creates the `transactional_templates` table (admin overrides for the automated system emails). ⚠ **Needs running on live** (created 24 July 2026); folded into `init*.sql`. Safe to run late — the mailer falls back to hardcoded defaults until it exists (and `loadTransactionalCache()` swallows the missing-table error). Needed before the Email Templates → Automated Emails tab can save an edit. See the mailer section.
...
- **`backend/config/init.sql`** — authoritative schema (31 tables) + seed data; re-runnable
...
**Editable automated (transactional) emails (24 July 2026, migration #21).** Five automated emails have an admin-editable subject + HTML body with `{{placeholders}}`: `registration_received`, `membership_approved`, `membership_rejected`, `password_reset`, `contact_acknowledgement`. Defaults are the source of truth in `mailerService.TRANSACTIONAL_TEMPLATES`; per-key overrides live in the `transactional_templates` table and are cached in memory (single Hostinger instance) — loaded at startup via `loadTransactionalCache()` (called in `server.js`) and refreshed after each save. `renderTransactional(key, vars)` interpolates (body values HTML-escaped, subject values raw) and the existing `templates.memberRegistered/memberApproved/memberRejected/passwordReset/contactAutoReply` delegate to it, so **all callers stay unchanged** and output is byte-identical to the old hardcoded templates when no override exists. Edited on the **Email Templates → Automated Emails** tab (`email-templates.html`); API `GET /api/admin/email/transactional`, `PUT /:key`, `DELETE /:key` (reset to default), all `authorizeAdmin`. `password_reset` edits are rejected if the body drops the `{{reset_link}}` placeholder. The other mailer templates (`memberNotice`, `adminEmail`, `contactStaffAlert`, `contactReply`) remain hardcoded (dynamic/internal body).
...
**SMS & comms:** `sms_logs`, `sms_templates`, `email_logs` (email send history/progress, mirrors `sms_logs`; admin Email Logs page), `email_templates` (reusable email subject + body; admin **Email Templates** page mirrors SMS Templates and is picked in the Send Email composer to fill subject + message — migration #20, 24 July 2026), `transactional_templates` (admin-editable overrides for the automated system emails — migration #21, 24 July 2026; see the mailer section)
