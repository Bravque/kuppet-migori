...
## Project Status (as of 28 July 2026)
...
**DB migrations — #1–27 applied on live; ⚠ #28 & #29 PENDING on live (run before/with the next deploy)**
Scripts #1–27 have been run on the live Hostinger DB and are folded into `init.sql` / `init-hostinger.sql` for fresh installs — their individual `backend/config/migration-*.sql` files are kept for re-provisioning reference (see `docs/SESSION-NOTES.md` for what each changed). Reference of the most recent:
29. `backend/config/migration-bbf-next-of-kin.sql` — death-of-member BBF claims + next of kin (7 September 2026). Adds to `bbf_claims`: `deceased_is_member` (1 = the member themselves died, admin-filed, vs. the normal relative-death claim), `next_of_kin_name`/`_relationship`/`_phone`/`_email` (the claimant — all claim notifications route here instead of the deceased member's own contacts), and `filed_by` (the admin who filed it, `users.id`; NULL for member-filed). Adds `members.is_deceased` (blocks login + closes the account; set when such a claim is filed). All nullable/defaulted; idempotent (`IF NOT EXISTS`) so re-running is safe. ⚠ **NOT yet applied on live** — run it on the live DB **before** the deploy that ships admin claim-filing, or `POST /api/admin/bbf` 500s (the new columns wouldn't exist). Folded into `init*.sql`.
28. `backend/config/migration-bbf-claim-form-doc.sql` — adds `bbf_claim_form` to the `bbf_claim_documents.doc_type` ENUM (10 August 2026). The death-claim slot formerly labelled **Letter From Principal** (`letter_from_principal`) is replaced by **BBF Claim Form** (`bbf_claim_form`): death claims now require TSC Slip + Burial Permit + BBF Claim Form (+ optional Birth Notification). `letter_from_principal` is kept in the ENUM + admin `DOC_LABELS` so documents on pre-existing death claims still render with their old label. ⚠ **NOT yet applied on live** — run it on the live DB **before** the deploy that ships the new slot, or BBF Claim Form uploads 500 (the ENUM would reject the value). Folded into `init*.sql`.
...
27. `backend/config/migration-member-disability.sql` — adds `members.has_disability TINYINT(1) DEFAULT 0` + `disability_description TEXT NULL` (Person With Disability / PWD). ✓ Applied 4 August 2026; folded into `init*.sql`. Captured at registration (required Yes/No radio + conditional description, required when Yes) and editable in profile; existing/imported members default to `0`/null. Shown on the admin member-detail page + members Excel export. `has_disability` is **not** a required onboarding field. The description is required *only* when the flag is set — enforced in `memberAuthController.register` (validator in `routes/memberAuth.js`) and in `memberProfileController.updateProfile` (handled as a pair, outside the `ALLOWED_FIELDS` loop: description required when flag on, cleared when off; only touched when the client sends one of the two keys).
...
- **`backend/services/notificationService.js`** — `createNotification()` inserts into `notifications` + optionally triggers SMS/email; also owns the editable **application-notification templates** (`NOTIFICATION_TEMPLATES` defaults + `renderNotification(key, vars)` + `loadNotificationCache()`) for BBF/scholarship status messages. The in-app row is always keyed to `memberId`, but `createNotification` accepts optional **`smsPhone`/`emailTo`/`recipientName`** overrides to deliver the SMS/email to someone other than the member (defaults to the member's own contacts when omitted) — used for **death-of-member BBF claims**, where the deceased member's phone/email are defunct and the next of kin must be notified (see the BBF data model)
...
**Members admin list (`members.html`).** Filters: status tabs · search (name·TSC·email) · **school** search (`school_name LIKE`) · sub-county · gender · **disability** (Person With Disability — `has_disability` `1`/`0`; any other value = all). All are built once in `adminMembersController.buildMemberFilter(req.query)`, shared by the list, the count, **and** the Excel export (`GET /api/admin/members/export`) so they stay in sync — the Export button posts the active filters. Columns include **ID No** (`national_id`). ⚠ `national_id` is the bulk-import **default password** until changed, so the ID No column (and member-detail) exposes those defaults to any admin viewing the screen.

**BBF Claims (`bbf.html`) + Scholarship Applications (`scholarship-apps.html`) list filters (24 July 2026; disability added 4 Aug 2026).** Both admin lists gained the same applicant filters as Members — **school category, sub-county, school (LIKE), gender, disability** — on top of their status tabs. They filter on the JOINed `members m` columns (`m.school_category`/`m.sub_county`/`m.school_name`/`m.gender`/`m.has_disability`), so gender and disability (neither stored on the claim/app) work too. BBF: added to `adminBbfController.buildBbfFilter` (so list, count **and** Excel export all share them; the Export button posts the active filters). Scholarship: added inline in `adminSchAppController.getAll`. Frontend filter-bar + debounced school search mirror the Members page. HTML-only + backend change (no versioned-asset bump).
...
**Profile-save semantics (28 July 2026) — two traps, both fixed in `memberProfileController.updateProfile`:**
- `updateProfile` returns **`just_onboarded`** (this save completed onboarding) *alongside* `onboarding_complete` (the stored flag). **Branch UI on `just_onboarded`** — `onboarding_complete` is true for every already-onboarded member, so branching on it made every ordinary profile edit announce "Profile complete!" and redirect to the dashboard.
- A **required** field may be filled in or corrected but **never cleared**: a blank over an existing value is a 400 naming the field; a blank over an already-blank field is skipped (so partial saves during onboarding still work). Empty strings used to be written as `NULL` while every validator was guarded with `value &&`, so a member could blank out phone/gender/job group and stay flagged complete. The required set is derived as `REQUIRED_PROFILE_FIELDS ∩ ALLOWED_FIELDS`; `employment_number` is the only editable non-required field and stays clearable.
- ⚠ The completeness flag is deliberately **one-way (0 → 1)**. Many live members predate `job_group` and are already flagged complete with it null — recomputing in both directions would lock them all back into onboarding.

**Demo member.** `node backend/scripts/create-demo-member.js [--name/--tsc/--id/--school …] [--out FILE]` emits phpMyAdmin SQL for a single member shaped exactly like an imported one (approved, national ID as password, `must_change_password=1`, `onboarding_complete=0`), taking the next `member_seq` inside the transaction and defaulting the school to the first active row in `schools` so it passes the curated-list check. `--reset` instead emits SQL that returns an existing demo member (matched by `--tsc`) to that state without consuming a member number — for re-walking the first-login flow. The generated `demo-member*.sql` are git-ignored (they carry bcrypt hashes). ⚠ A reset does **not** invalidate the member's existing JWT — log out first.
...
- `members.has_disability` (`TINYINT(1)` DEFAULT 0) + `members.disability_description` (`TEXT` NULL): Person With Disability (PWD). Captured at registration + editable in profile; description required only when the flag is set (not a required onboarding field) — migration #27
...
The new-claim form (`member/bbf-claims.html`) shows the member's identity in a read-only "Your details (from your profile)" box; the member only chooses type + (for death) deceased name/relationship/date of death. `date_of_death` may not be in the future (shared `deathFieldError()`, applied in both `create` and `update`; today is accepted, compared against end-of-day so a timezone offset can't reject it).

**Death of the member themselves — admin-filed (7 September 2026, migration #29).** The two flows above are member-initiated (a living member claims for a relative, or for their own retirement). When the **member has died**, an admin files the death-benefit claim on their behalf via `POST /api/admin/bbf` → `adminBbfController.createForMember` (`authorizeAdmin`, multipart, "File Death Claim" button on `bbf.html`). It's a `death` claim with **`deceased_is_member = 1`**: `deceased_name` = the member's own `full_name`, the identity snapshot is pulled from the member row exactly like `memberBbfController.create`, and a **next of kin** (`next_of_kin_name`/`_relationship`/`_phone`/`_email`) is recorded as the claimant. The required death documents (TSC Slip + Burial Permit + BBF Claim Form; Birth Notification optional) are attached in the same request (`upload.bbfDocs.fields`), the claim is created straight at **`submitted`** (no draft stage — the desk files it complete), the member's account is flagged **`is_deceased = 1`** in the same transaction (blocks login, see `memberAuthController.login`), and a timeline row + the `bbf_submitted` notification are written. **One open death-of-member claim per member** (a `rejected` one may be refiled). All status notifications (submitted → paid) route to the **next of kin, not the deceased member** — see the notifications section (`createNotification` recipient overrides + `nokTarget()`). `date_of_death` not-in-future is enforced by `dodError()` in `adminBbfController`. `bbf-detail.html` renders a "Death of member" badge + a Deceased / Next-of-Kin split (both on screen and in the print layout).

**Document rules** — `BBF_DOC_SLOTS` in `memberBbfController` is the **single source of truth** for both which `doc_type`s a claim may carry and which are required to submit (28 July 2026); `submitClaim` derives its required list *and* its labels from it, and it mirrors `BBF_DOC_SLOTS_DEATH`/`_RETIREMENT` in `member-portal.js` — keep the two in step. **death** → TSC Slip, Burial Permit, BBF Claim Form (+ optional Birth Notification); **retirement** → TSC Slip + Letter of Compulsory Retirement. `letter_of_compulsory_retirement` was added to the `bbf_claim_documents.doc_type` ENUM in migration #19 and `bbf_claim_form` in migration #28 (10 Aug 2026, replacing the old `letter_from_principal` death slot — `letter_from_principal` is retained in the ENUM + `DOC_LABELS` for legacy rows); admin `bbf-detail.html` `DOC_LABELS` maps them too. `uploadDocuments` enforces:
- **Draft only.** Documents lock once the claim leaves `draft` (400) — they're the basis the decision is made on, and a later change would leave no trace in the timeline. The member UI already hides the controls; this is the guard behind it.
- **`doc_type` validated** against the slots for *that claim type* (+ the free-form `other`). It lands in an ENUM column, where an unrecognised value otherwise errors as a bare 500 or — on a non-strict server — is coerced to `''`, leaving a document that looks uploaded but can never satisfy the required check.
- **A slot holds exactly one document.** Re-uploading replaces (old row deleted in the transaction, old file unlinked after commit); several files for one slot is a 400. `other` appends. Before 28 July it appended, so the member's page showed the first upload while an admin saw both with no way to tell which was current.

### Scholarship applications data model
No draft stage — an application is created atomically at `applied` (row + both documents + the opening timeline entry in one transaction). The applicant is the logged-in member; the form collects **only the two required uploads** (Letter of Application + TSC Slip), so `institution`/`course`/`year_of_study`/`academic_year`/`essay` are left NULL.
- **One application per member per scholarship** — enforced by `UNIQUE(member_id, scholarship_id)` (migration #17). `apply` catches `ER_DUP_ENTRY` on that key → 409; the read-then-write check in front of it is a courtesy, not the guarantee.
- **The deadline is enforced** (28 July 2026). `IS_CLOSED_SQL` (`application_deadline IS NOT NULL AND application_deadline < CURDATE()`) is evaluated *by MySQL* so the API and the site can't disagree about "today"; the deadline day itself is still open and a null deadline is open-ended. `getAvailable` returns it as `is_closed` and the member portal renders a disabled "Applications closed" button. Previously only `is_active` was checked, so the portal happily accepted applications for a scholarship the public site already showed as expired.
- **Documents lock at `applied`.** `reuploadDocument` swaps one required document (transactional: old row deleted, **old file unlinked after commit**) and 400s once review has started.

### Status-decision race (BBF + scholarships)
Every admin decision endpoint reads the row, checks the transition, then updates. The check alone is TOCTOU — a transaction doesn't help, since the `SELECT` isn't `FOR UPDATE`. **Make the `UPDATE` conditional on the status just read and check `affectedRows`**, throwing `StaleClaim` (`adminBbfController`) / `StaleApplication` (`adminSchAppController`) → **409** so the admin reloads rather than seeing a bare 500. Without it two admins — or one double-click — both write a timeline row and fire a notification. The member-side `submitClaim` does the same (`WHERE id = ? AND status = 'draft'`); there it mattered most, since the member portal has no button-busy guard and a duplicate submit meant two SMS *and* two emails.
...
### File uploads
Files are stored under the upload root with UUID filenames (multer **2.x**) — **the extension is sanitised**: `safeExtension()` in `middleware/upload.js` accepts only `/^\.[a-z0-9]{1,10}$/` and otherwise stores the file without one. The uploader controls `originalname`, and `path.extname` returns everything after the last dot, so an unsanitised extension could carry quotes into `file_url` and out into anything rendering it (this was a live member→admin XSS — see the `20260728c` cache-token note). Type is enforced by `fileFilter` on the **MIME type**, never the name.

**Orphan cleanup — `backend/utils/uploads.js`.** Multer (diskStorage) writes every accepted file *before* the route handler runs and cleans up only when **it** fails (size limit, rejected type); anything that rejects afterwards used to strand the files on the persistent `UPLOAD_DIR` forever. `removeUploadedFiles(req)` is wired into `handleValidation` (covers every multer+validator route) and the global error handler in `server.js`, and controllers that catch their own errors call it on each reject path — **a controller that responds without recording an upload must call it itself**, since neither shared hook will see that request. `removeStoredFile(fileUrl)` deletes an already-stored file (used when a document is superseded); call it only after the DB row is gone. Both are best-effort and never throw.

Files live in subdirs `photos/`, `documents/`, `bbf/`, `scholarships/`, `members/`, `news/`, `court/`, `disciplinary/`. The sensitive subdirs — `members/`, `bbf/`, `scholarships/`, `court/`, `disciplinary/` — are **404-blocked from static serving** in `server.js` (before the `/uploads` static); only `photos/`, `documents/`, `news/` are public. Sensitive files are streamed only through ownership/role-checked endpoints:
- **Members:** `GET /api/member/documents/:filename` — verifies the file belongs to the logged-in member.
- **Admins:** `GET /api/admin/documents/:filename` (`backend/routes/adminDocuments.js`) — `authenticate` + `authorizeAdmin`; admin detail pages fetch these as a blob via `viewDoc()` in `admin-portal.js` (a plain link can't send the Bearer token), triggered by a **`data-view-doc="<file_url>"`** attribute on the button and one delegated listener — never an inline `onclick`, see the `20260728c` note.
