...
`backend/services/mailerService.js` sends via nodemailer when `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` are set (else it logs & skips). `sendMail()` supports an optional `replyTo`. **Every outgoing email is wrapped in a branded shell** (24 July 2026) by `brandEmail(bodyHtml)` inside `sendMail` — table-based + inline-styled for email-client compat: logo header (absolute URL `${APP_URL}/images/kuppetlogo.png`), a `#1B3A6E` "KUPPET MIGORI BRANCH" band, a `#C8962A` gold accent line, the body fragment, and a footer (address · phone · email · WhatsApp channel). Templates only produce the inner body fragment; the shell is applied once centrally. The plain-text alt part is derived from the fragment (before wrapping). `APP_URL` (e.g. `https://kuppetmigori.co.ke`) is used to build links in emails (password reset). Live uses the Hostinger mailbox for `info@kuppetmigori.co.ke` (`smtp.hostinger.com:465`). Test with `node backend/scripts/test-email.js you@example.com`.
